...
**Implementado** (flujo cableado; smoke sin el bot verificado 2026-09-10): `POST /v1/telegram/link-code` (api) → `/link CODE` en el bot verifica el `TelegramLink`. Foto → bot sube a R2 (`@fijaprecio/storage`, MinIO local) → `Receipt(TELEGRAM)` → cola `ocr.parse`. Worker `OcrProcessor`: `storage.get` → `POST {OCR_URL}/internal/ocr` → `parseReceipt` → match por línea (`GET {API_URL}/v1/internal/catalog/match`) → persiste `ReceiptLineItem[]` → `POST {BOT_URL}/internal/notify` (falla → warn, no pierde el parseo). Bot manda el resumen con botones ✅/❌ → confirmar llama `POST /v1/internal/receipts/:id/confirm` → `PriceObservation(source=OCR)` + `OrgInput.lastKnownPrice` + recálculo de consenso. **Verificado**: boleta sintética → OCR (Tesseract) → parse → worker → `Receipt.CONFIRMED` + `PriceObservation(OCR)`. Tesseract lee mal dígitos en imágenes densas (RUC/fecha/IGV fallan) → producción con PaddleOCR / OCR cloud (adaptador `OcrProvider`). Falta el bot con token real de Telegram.
...
- Contexto por request: **`OrgContextMiddleware`** (`apps/api/src/tenancy/`) valida el JWT y abre el `AsyncLocalStorage` de tenant con `orgContextStorage.run()` envolviendo `next()` — un **middleware**, no un guard, porque `enterWith` desde un guard NO se propaga de forma fiable a los servicios (bug encontrado al activar RLS: todo corría como `bypass_rls`). `JwtAuthGuard` queda como puro enforcement (401). **Implementado y verificado.**
- Refuerzo: **Postgres RLS** — **ACTIVO** (`DB_RLS_ENFORCED=true` en local desde 2026-09-09). Políticas `tenant_isolation` en `20260907235300_manual_*` (21 tablas de tenant + `PaymentEvent`). El rol `fijaprecio_app` NOBYPASSRLS lo crea `20260908113000_rls_app_role` (+ grants + ALTER DEFAULT PRIVILEGES); en local se le da LOGIN con `pnpm db:rls:setup` (password `fijaprecio_app`). Con `DB_RLS_ENFORCED=true` + `APP_DATABASE_URL`, la **API** conecta con ese rol y la extensión Prisma `rls` (`apps/api/src/prisma/rls.extension.ts`) envuelve cada op de modelo en una tx de 2 sentencias que fija `app.current_org` (contexto de tenant) o `app.bypass_rls=on` (sin contexto → system/internal/auth). `@RlsSystem()` quedó **vestigial** (informativo): sin JWT el middleware no abre contexto y la extensión ya trata esas queries como system. Transacciones interactivas y raw sobre tablas de tenant van por `PrismaService.withRls(fn, orgId?)` / `asSystem(fn)` (cliente base, sin la extensión, bajo `rlsReentry`). `worker`/`bot` siguen como el dueño. El scoping primario sigue siendo el `where: { organizationId }`; RLS es la red de seguridad. Verificación: `pnpm db:rls:check` + smoke end-to-end (aislamiento org A≠B, writes, endpoints internos, register, web SSR — todo verde). Ver `packages/db/prisma/README.md §4b`.
