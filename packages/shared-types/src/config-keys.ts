 ...
 *
 * Lo consumen `AppConfigService` (api) y el motor de consenso (worker). Añadir
 * una clave => añadirla también al seed.
 */
...
export const CONFIG_SCHEMAS = {
  ...
  'consensus.reputation_full_weight_at': z.number().positive(),
  'consensus.confidence_weights': z.object({
    size: z.number().min(0),
    dispersion: z.number().min(0),
    diversity: z.number().min(0),
  }),
  ...
  // --- catálogo / matching -------------------------------------------------
  'catalog.match_min_similarity': fraction, // umbral para SUGERIR en autocompletado
  'catalog.match_auto_assign_similarity': fraction, // umbral para asignar sin preguntar
  'catalog.autocreate_canonical_status': z.enum(['ACTIVE', 'PENDING_REVIEW']),
  ...
  'notify.dedup_hours': z.number().positive(),
